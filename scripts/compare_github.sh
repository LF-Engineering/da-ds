...
# _ID=kubernetes-client/gen/pull/1
...
# curl -s "${ES2_URL}/sds-cncf-k8s-github-pull_request/_search" | jq '.hits.hits[]._source.url_id'
...
# ES1_URL='http://127.0.0.1:19200' ES2_URL="`cat ../sync-data-sources/helm-charts/sds-helm/sds-helm/secrets/ES_URL.prod.secret`" _ID=kubernetes-client/gen/pull/1 ./scripts/compare_github.sh
if [ -z "${ES1_URL}" ]
then
...
curl -s -H 'Content-Type: application/json' "${ES1_URL}/sds-da-ds-gh-api-github-issue/_search" -d "{\"query\":{\"term\":{\"url_id\":\"${_ID}\"}}}" | jq -rS '.' > dads.json
#curl -s -H 'Content-Type: application/json' "${ES2_URL}/sds-cncf-k8s-github-issue/_search" -d  "{\"query\":{\"term\":{\"url_id\":\"${_ID}\"}}}" | jq -rS '.' > p2o.json
curl -s -H 'Content-Type: application/json' "${ES2_URL}/sds-cncf-k8s-github-pull_request/_search" -d  "{\"query\":{\"term\":{\"url_id\":\"${_ID}\"}}}" | jq -rS '.' > p2o.json
