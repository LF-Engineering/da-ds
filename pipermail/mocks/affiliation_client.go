// Code generated by mockery v0.0.0-dev. DO NOT EDIT.

package mocks

import (
	affiliation "github.com/LF-Engineering/dev-analytics-libraries/affiliation"
	mock "github.com/stretchr/testify/mock"
)

// AffiliationClient is an autogenerated mock type for the AffiliationClient type
type AffiliationClient struct {
	mock.Mock
}

// AddIdentity provides a mock function with given fields: identity
func (_m *AffiliationClient) AddIdentity(identity *affiliation.Identity) bool {
	ret := _m.Called(identity)

	var r0 bool
	if rf, ok := ret.Get(0).(func(*affiliation.Identity) bool); ok {
		r0 = rf(identity)
	} else {
		r0 = ret.Get(0).(bool)
	}

	return r0
}

// GetIdentityByUser provides a mock function with given fields: key, value
func (_m *AffiliationClient) GetIdentityByUser(key string, value string) (*affiliation.AffIdentity, error) {
	ret := _m.Called(key, value)

	var r0 *affiliation.AffIdentity
	if rf, ok := ret.Get(0).(func(string, string) *affiliation.AffIdentity); ok {
		r0 = rf(key, value)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*affiliation.AffIdentity)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(string, string) error); ok {
		r1 = rf(key, value)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// GetOrganizations provides a mock function with given fields: uuid, projectSlug
func (_m *AffiliationClient) GetOrganizations(uuid string, projectSlug string) *[]affiliation.Enrollment {
	ret := _m.Called(uuid, projectSlug)

	var r0 *[]affiliation.Enrollment
	if rf, ok := ret.Get(0).(func(string, string) *[]affiliation.Enrollment); ok {
		r0 = rf(uuid, projectSlug)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*[]affiliation.Enrollment)
		}
	}

	return r0
}
