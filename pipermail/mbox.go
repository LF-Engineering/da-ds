...
func ParseMBoxMsg(Debug int, groupName string, msg []byte) (item map[string]interface{}, valid, warn bool) {
	...
	addRaw := func(k string, v []byte, replace int) {
		...
		if len(raw) >= MaxMessageProperties {
			return
		}
		a, ok := raw[k]
		if ok {
			...
			case 2:
				raw[k] = [][]byte{v}
			default:
				fmt.Printf("addRaw called with an unsupported replace mode(%s,%d)\n", groupName, len(msg))
			}
			return
		}
		raw[k] = [][]byte{v}
	...
	lines := bytes.Split(msg, MessageLineSeparator)
	...
	pop := func() {
		...
		if n < 0 {
			fmt.Printf("%s(%d): cannot pop from an empty stack\n", groupName, len(msg))
			warn = true
			return
		}
	...
	for idx, line := range lines {
		...
		if len(line) == 0 {
			if !body {
				...
				if bytes.Contains(contentType, boundarySep) {
					...
					if len(boundary) == 0 {
						fmt.Printf("#%d cannot find multipart message boundary(%s,%d) '%s'\n", i, groupName, len(msg), string(contentType))
						warn = true
					}
				...
				if ok {
					...
					if lowerKey == LowerContentType {
						...
						if bytes.Contains(currContentType, boundarySep) {
							...
							if len(boundary) == 0 {
								fmt.Printf("#%d cannot find multiboundary message boundary(%s,%d)\n", i, groupName, len(msg))
								warn = true
							}
						}
						continue
		...
		if cont {
			if currKey == "" {
				fmt.Printf("#%d no current key(%s,%d)\n", i, groupName, len(msg))
				warn = true
				break
			}
			currVal, ok := getRaw(currKey)
			if !ok {
				fmt.Printf("#%d missing %s key in %v\n", i, currKey, lib.DumpKeys(raw))
				warn = true
				break
			}
			...
			key, val, ok := getHeader(i, line)
			if !ok {
				fmt.Printf("#%d incorrect header(%s,%d)\n", i, groupName, len(msg))
				warn = true
				break
			}
			addRaw(key, val, 0)
	...
	for k := range raw {
		...
		if lk == MessageIDField || lk == MessageDateField {
			item[lk] = sv
			if lk != k {
				ks = append(ks, lk)
			} else {
		...
		if lk == MessageReceivedField && lk != k {
			raw[lk] = raw[k]
		}
		ks = append(ks, k)
	}
	if Debug > 2 {
		sort.Strings(ks)
		for i, k := range ks {
			if k == MessageReceivedField || k == MessageIDField || k == MessageDateField {
				fmt.Printf("#%d %s: %v\n", i+1, k, item[k])
			} else {
				a, ok := item[k].([]string)
				if ok {
	...
	_, ok := item[MessageIDField]
	if !ok {
		fmt.Printf("%s(%d): missing Message-ID field\n", groupName, len(msg))
		dumpMBox()
		return
	}
	var (
	...
	found := false
	mdt, ok := item[MessageDateField]
	if !ok {
		rcvs, ok := raw[MessageReceivedField]
		if !ok {
			fmt.Printf("%s(%d): missing Date & Received fields\n", groupName, len(msg))
		}
		type DtTz struct {
			Dt   time.Time
			DtTz time.Time
		...
		for _, rcv := range rcvs {
			...
			sdt := ary[len(ary)-1]
			dt, dttz, tz, ok := lib.ParseDateWithTz(sdt)
			if ok {
		...
		nDts := len(dts)
		if nDts == 0 {
			fmt.Printf("%s(%d): missing Date field and cannot parse date from Received field(s)\n", groupName, len(msg))
			dumpMBox()
			return
		}
		if nDts > 1 {
	...
	if !found {
		sdt, ok := mdt.(string)
		if !ok {
			fmt.Printf("%s(%d): non-string date field %v\n", groupName, len(msg), mdt)
		}
		dt, dttz, tz, ok = lib.ParseDateWithTz(sdt)
		if !ok {
			fmt.Printf("%s(%d): unable to parse date from '%s'\n", groupName, len(msg), sdt)
			dumpMBox()
			return
		}
	}
	...
	for i, body := range bodies {
		...
		sBody := lib.BytesToStringTrunc(body.Data, MaxMessageBodyLength, false)
		...
		_, ok := bodyKeys[key]
		if !ok {
			lib.FatalOnError(lib.DeepSet(item, path, []interface{}{m}, true))
			bodyKeys[key] = struct{}{}
		} else {
			iface, _ := lib.Dig(item, path, true, false)
			ifary, _ := iface.([]interface{})
			ifary = append(ifary, m)
			lib.FatalOnError(lib.DeepSet(item, path, ifary, true))
		}
		lib.Printf("#%d: %s %s %d\n", i, string(body.ContentType), propertiesString(body.Properties), len(body.Data))
	}
	if DropXFields {
